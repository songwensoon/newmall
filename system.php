...
class System extends IController
{
    ...
    */
	...
	//[权限管理][管理员]检查admin_no唯一性
	function check_admin_no($no = null,$id = null)
	{
		//php校验$name!=null , ajax校验 $name == null
		$admin_no = ($no==null) ? IReq::get('admin_no','post') : $no;
		$admin_id   = ($id==null)   ? IReq::get('admin_id','post')   : $id;
		$admin_no = IFilter::act($admin_no);
		$admin_id = intval($admin_id);
	
		$adminObj = new IModel('admin');
		if($admin_id)
		{
			$where = 'admin_no = "'.$admin_no.'" and id != '.$admin_id;
		}
		else
		{
	...
	function admin_edit_act()
	{
		...
		$dataArray = array(
			...
			'admin_no' => IFilter::string( IReq::get('admin_no','post') ),
		...
		//检查管理员name唯一性
		$isPass = $this->check_admin_no($dataArray['admin_no'],$id);
		if($isPass == false)
		{
			$message = $dataArray['admin_no'].'员工号已经存在,请更改员工号';
		}
		//提取密码 [ 密码设置 ]
		...
		if($id)
		{
			...
			else
			{
				...
				if($id == $this->admin['admin_id'])
				{
					...
					ISafe::set('admin_no',$dataArray['admin_no']);
		...
		else
		{
			...
			else
			{
				...
				$adminObj->setData($dataArray);
				$admin_id = $adminObj->add();
				if($admin_id)
				{
					$staff_id = IReq::get('staff_id','post');
					if(!empty($staff_id))
					{
	...
	function role_edit()
	{
		...
		$rightData = $rightObj->query('`is_del` = 0 and `is_show` = 1','*','name','asc');
		$rightArray     = array();
	...
	{
		$id = IReq::get('id');
		$role_id = IReq::get('role_id');
		$data = $this->dispatchstaff($role_id,$id);
		echo JSON::encode($data);
	}

	/**
	 * @brief 分配员工
	 * $role_id 角色 2是 员工 3是主管 4 是经理
	 */
	public function dispatchstaff($role_id,$admin_id='')
	{
		$staff_obj = new IModel('staff_relations');
		...
		if($role_id == 3){
			...
			$where = '';
			$where1 = '';
			$staff_id = array(); //已存在id
			$id_str = '';
			if($admin_id){
				$where = 'parent_id!='.$admin_id;
				$where1 = 'parent_id='.$admin_id;
				$staff_id[] = $admin_id;
			}
			$data_staff = $staff_obj->query($where);
			$exists_staff = $staff_obj->query($where1);
			$exists_data = array();
			if(!empty($exists_staff)){
				foreach($exists_staff as $v){
					$exists_data[] = $v['admin_id'];
				}
			}
			if(!empty($data_staff))
			{
				foreach($data_staff as $key=>$staff)
				{
			...
			if(!empty($staff_id))
			{
				$id_str = join(",",$staff_id);
			}
			$admin_obj = new IModel('admin');
			$where = ' role_id=2 ';
			if(!empty($id_str))
			{
				$where .= ' and id not in ('.$id_str.')';
			}
			$data_admin = $admin_obj->query($where);
			$data = array(
				'result'=>'00000000',
				'data_admin'=>$data_admin,
				'data_staff'=>$data_staff,
				'exists_staff'=>$exists_data
			);
		}else if($role_id == 4){//如果是经理则分配主管
			//获取已经有关联的员工
			$where = '';
			$where1 = '';
			$staff_id = array(); //已存在id
	 ...
	 * 
	 */
	public function set_relations($admin_id=null,$parent_id=null)
	{
		$parent_id = ($parent_id==null) ? IReq::get('parent_id','post') : $parent_id;
		$admin_id   = ($admin_id==null) ? IReq::get('admin_id','post')   : $admin_id;
		$admin_id = intval($admin_id);
		$parent_id = intval($parent_id);
		$staff_obj = new IModel('staff_relations');
		$where = ' admin_id='.$admin_id.' and parent_id='.$parent_id;
		$data_staff = $staff_obj->query($where);
		if(!empty($data_staff))
		{
			$staff_obj->del($where);
